...
untracked file never overwrites a file that currently exists on disk, including
files or symlinks that obstruct a captured path's parent directories. If manual
recovery is needed, extract the capture with `git archive` into a new, empty
directory. The stash
...
Failed snapshot attempts and commands with snapshots disabled clear the current
rollback binding, so a reused server engine cannot inherit an earlier command's
snapshot.
