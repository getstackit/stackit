...
func TestUpdatePRBodyFooter(t *testing.T) {
	...
	t.Run("replaces existing stackit section with markers", func(t *testing.T) {
		body := "Description\n\n" + actions.SectionStart + "\nOld Content\n" + actions.SectionEnd + "\nFooter stuff"
		newFooter := "\n\n\n" + actions.SectionStart + "\nNew Content\n" + actions.SectionEnd
		result := actions.UpdatePRBodyFooter(body, newFooter)
	...
	t.Run("idempotency - multiple updates", func(t *testing.T) {
		body := "Description"
		first := actions.UpdatePRBodyFooter(body, footer)
		second := actions.UpdatePRBodyFooter(first, footer)

		assert.Equal(t, first, second)
		assert.Equal(t, 1, strings.Count(second, actions.SectionStart))
	})
...
func TestCreatePRBodyFooter(t *testing.T) {
	t.Run("includes lock instructions when branch is locked", func(t *testing.T) {
		...
		footer := actions.CreatePRBodyFooter("feature-a", s.Engine)
