...
func TestPruneRefusal(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			entry: Entry{AnchorBranch: "anchor", Lifecycle: WorktreeLifecycle{Presence: WorktreePresent, Remove: LifecycleAllowed}},
		},
		{
			name:  "missing directory is prunable",
			entry: Entry{AnchorBranch: "anchor", Lifecycle: WorktreeLifecycle{Presence: WorktreeMissing, Remove: LifecycleAllowed}},
		},
		{
			name: "missing directory with stacked branches is refused",
			// Reported prunable by --dry-run, then refused by the real run.
			entry:   Entry{AnchorBranch: "anchor", RootBranches: []string{"feature"}, StackSize: 1, Lifecycle: WorktreeLifecycle{Presence: WorktreeMissing, Remove: LifecycleHasBranches}},
			refused: true,
		},
		{
		...
		},
		{
			...
			entry: Entry{
				...
				Lifecycle: WorktreeLifecycle{
					...
					Registration: RegistrationStateLegacy,
		...
		},
		{
			...
			entry:   Entry{AnchorBranch: "anchor", Lifecycle: WorktreeLifecycle{Presence: WorktreePresent, Remove: LifecycleCurrent}},
			refused: true,
		},
		{
			name:    "dirty worktree is refused",
			entry:   Entry{AnchorBranch: "anchor", Lifecycle: WorktreeLifecycle{Presence: WorktreePresent, Changes: WorktreeChangesDirty, Remove: LifecycleAllowed}},
			refused: true,
		},
		{
			name: "dirty flag on a missing directory does not refuse",
			// Nothing on disk to protect: the flag is stale.
			entry: Entry{AnchorBranch: "anchor", Lifecycle: WorktreeLifecycle{Presence: WorktreeMissing, Changes: WorktreeChangesDirty, Remove: LifecycleAllowed}},
		},
	}
