...
func runMergeSquash(ctx *app.Context, opts mergeSquashOptions, postMergeHandler PostMergeHandler) error {
	...
	if !opts.yes && ctx.Interactive && len(plan.BranchesToMerge) > 0 {
		merged, err := tryIndividualMerge(ctx, opts, plan, postMergeHandler)
		if err != nil {
			return err
		}
		if merged {
...
func tryIndividualMerge(ctx *app.Context, opts mergeSquashOptions, plan *mergeAction.Plan, postMergeHandler PostMergeHandler) (bool, error) {
	out := ctx.Output
