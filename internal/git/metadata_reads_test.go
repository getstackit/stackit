...
func TestMetadataReadsPreservePartialFailures(t *testing.T) {
	...
	require.ErrorContains(t, shared.Failures()["corrupt"], "unmarshal")
	record, err := shared.Record("corrupt")
	require.ErrorContains(t, err, "unmarshal")
	require.Equal(t, corrupt, record.SHA)
	missing, err := shared.Record("missing")
	require.NoError(t, err)
	...
	require.ErrorContains(t, local.Failures()["corrupt"], "unmarshal")
	localRecord, err := local.Record("corrupt")
	require.ErrorContains(t, err, "unmarshal")
	require.Equal(t, corrupt, localRecord.SHA)
