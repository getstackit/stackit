...
func TestMetadataReadsPreservePartialFailures(t *testing.T) {
	...
	local := runner.ReadLocalMetadata(t.Context(), "valid", "missing", "corrupt")
	...
	_, err = runner.ReadLocalMetadata(t.Context(), "corrupt").One()
	require.ErrorContains(t, err, "unmarshal")
	ctx, cancel := context.WithCancel(t.Context())
	cancel()
	_, err = runner.ReadMetadata(ctx, "valid").One()
	require.ErrorIs(t, err, context.Canceled)
	_, err = runner.ReadLocalMetadata(ctx, "valid").One()
	require.ErrorIs(t, err, context.Canceled)
}
