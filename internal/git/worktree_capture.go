...
func (r *runner) CaptureUntracked(ctx context.Context, message string) (string, error) {
	...
	for chunk := range slices.Chunk(paths, restorePathBatchSize) {
		...
		addArgs := append([]string{gitLiteralPathspecs, gitCmdAdd, "--"}, chunk...)
...
func (r *runner) RestoreUntracked(ctx context.Context, commitSHA string) (int, error) {
	...
	for _, path := range strings.Split(output, "\x00") {
		...
		safe, err := canRestoreUntrackedPath(root, path)
		if err != nil {
			return 0, err
		}
		if !safe {
	...
	for chunk := range slices.Chunk(missing, restorePathBatchSize) {
		args := append([]string{gitLiteralPathspecs, "restore", "--source", commitSHA, "--worktree", "--"}, chunk...)
...
// Inspect ancestors first: Git replaces obstructing files and symlinks when
// restoring a descendant. Only real directories may be traversed, and an
// inspection error must not be mistaken for a missing path.
func canRestoreUntrackedPath(root, path string) (bool, error) {
	...
	for i, part := range parts {
		...
		if err != nil {
			return false, fmt.Errorf("failed to inspect captured path %q: %w", path, err)
		}
