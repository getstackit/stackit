...
func TestRestoreUntrackedPreservesObstructingAncestors(t *testing.T) {
	t.Parallel()
	for _, kind := range []string{"file", "directory symlink", "dangling symlink"} {
		t.Run(kind, func(t *testing.T) {
			...
			scene := testhelpers.NewSceneParallel(t, testhelpers.InitialCommitSceneSetup)
			runner := git.NewRunnerWithPath(scene.Dir, nil)
			parent := filepath.Join(scene.Dir, "notes")
			require.NoError(t, os.Mkdir(parent, 0o750))
			require.NoError(t, os.WriteFile(filepath.Join(parent, "old.txt"), []byte("captured"), 0o600))
			...
			switch kind {
			case "file":
				require.NoError(t, os.WriteFile(parent, []byte("new work"), 0o600))
			...
			if kind == "file" {
				...
				require.NoError(t, err)
				require.Equal(t, "new work", string(content))
			} else {
				link, err := os.Readlink(parent)
				require.NoError(t, err)
				require.Equal(t, target, link)
				require.NoFileExists(t, filepath.Join(target, "old.txt"))
			}
		})
	}
}
