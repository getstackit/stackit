...
func TestRollbackPreservesIntentToAdd(t *testing.T) {
	t.Parallel()
	for _, operation := range []string{"modify", "create", "abort"} {
		t.Run(operation, func(t *testing.T) {
			t.Parallel()
			sh := NewTestShellInProcess(t)
			sh.WriteFile("existing.txt", "base\n").Run("create a -m 'a'")
			if operation == "abort" {
				sh.WriteFile("existing.txt", "child\n").Run("create b -m 'b'").Checkout("a")
			}
			original := sh.revParse("a")
			sh.WriteUnstaged("existing.txt", "edited\n").WriteUnstaged("new.txt", "new work\n").Git("add -N new.txt")
			before := sh.gitStatus()
			switch operation {
			case "modify":
				sh.Run("modify -a").Run("undo -y")
			case "create":
				sh.Run("create b -a -m 'b'").Run("undo -y")
			case "abort":
				sh.RunExpectError("modify -a").OutputContains("Conflicted files")
				sh.Run("abort --force")
			}
			sh.OnBranch("a")
			require.Equal(t, original, sh.revParse("a"))
			require.Equal(t, before, sh.gitStatus())
			require.Equal(t, "edited\n", sh.fileContent("existing.txt"))
			require.Equal(t, "new work\n", sh.fileContent("new.txt"))
		})
	}
}

func TestAbortRetriesAfterUntrackedCollision(t *testing.T) {
	t.Parallel()
	for _, attached := range []bool{true, false} {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			sh := NewTestShellInProcess(t)
			sh.WriteFile("shared.txt", "a\n").WriteFile("victim", "original\n").Run("create a -m 'a'")
			sh.WriteFile("shared.txt", "b\n").Run("create b -m 'b'")
			sh.Checkout("a")
			original := sh.revParse("a")
			sh.Git("rm victim").WriteUnstaged("shared.txt", "amended\n")
			...
			amended := sh.revParse("a")

			// Unwind Git first, leaving stackit's rollback pending. The user
			// may or may not have checked out their branch again.
			sh.Git("rebase --abort")
			if attached {
				sh.Checkout("a")
			} else {
				sh.Git("checkout --detach a")
			}
			sh.WriteUnstaged("victim", "new work\n").RunExpectError("abort --force").OutputContains("untracked")
			require.Equal(t, amended, sh.revParse("a"))
			require.Equal(t, "new work\n", sh.fileContent("victim"))
			pending, err := config.GetContinuationState(sh.Dir())
			require.NoError(t, err, "a refused rollback must remain retryable")
			require.Equal(t, state.SnapshotID, pending.SnapshotID)

			require.NoError(t, os.Remove(filepath.Join(sh.Dir(), "victim")))
			sh.Run("abort --force").OnBranch("a")
			require.Equal(t, original, sh.revParse("a"))
			require.Equal(t, statusBefore, sh.gitStatus())
			_, err = config.GetContinuationState(sh.Dir())
			require.Error(t, err, "successful abort must clear the pending rollback")
		})
	}
}
