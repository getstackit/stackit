...
// FileError writes an error message only to the log file, not to the console
func (s *Splog) FileError(format string, args ...interface{}) {
	if s.fileLogger == nil {
		return
	}
	var msg string
...
// LogPanic logs a panic and its stack trace to the default log file
func LogPanic(r interface{}, stack []byte) {
	logPath := GetLogFilePath()
	splog, _ := NewSplogWithFlags(logPath, false, true)
	if splog != nil {
		splog.FileError("Panic recovered: %v\n%s", r, string(stack))
		_ = splog.Close()
	}
}

// SafeGo runs the given function in a new goroutine and logs any panic that occurs.
func SafeGo(fn func()) {
	go func() {
		defer func() {
			if r := recover(); r != nil {
				LogPanic(r, debug.Stack())
				// If a restorer is registered, use it
...
// TerminalRestorer is an optional function that restores the terminal state.
// This is set by the utils package to avoid circular imports.
